...
export const toSuchparameter = (param?: SuchParameterInput): Suchparameter | undefined => {
  if (param === undefined) {
    return undefined;
  }

  const { username, email, level, experience, playerClass, status, guildId, guild } = param;
...
export const toCreate = (player: PlayerNeuInput): PlayerCreate => {
  const { username, email, level, experience, playerClass, status, guildId } = player;
