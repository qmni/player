...
export class PlayerService {
  ...
  async #findAll(pageable: Pageable): Promise<Readonly<Slice<PlayerMitGuild>>> {
    const { number, size } = pageable;
  ...
  #createSlice(players: PlayerMitGuild[], totalElements: number): Readonly<Slice<PlayerMitGuild>> {
  ...
  #checkEnums(suchparameter: Suchparameter) {
    ...
    this.#logger.debug('#checkEnums: playerClass=%s, status=%s', playerClass, status);
    ...
    const validStatus =
      status === undefined || status === 'ACTIVE' || status === 'BANNED' || status === 'DELETED';
